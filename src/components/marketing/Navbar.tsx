    ...
    <header className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[94%] max-w-5xl">
      ...
      >
        ...
        <div className="flex items-center gap-5">
          <div className="hidden sm:flex items-center">
            <Link href="/login" className="text-[14px] font-semibold text-ng-secondary hover:text-ng-text transition-colors mr-5">
              Log in
            </Link>
          </div>
          <button
      ...
      {menu && (
        ...
        >
          ...
          <Link href="/login" className="font-display font-semibold text-ng-text text-[14px]" onClick={() => setMenu(false)}>
            Log in to Console
          </Link>
        </div>
      )}
